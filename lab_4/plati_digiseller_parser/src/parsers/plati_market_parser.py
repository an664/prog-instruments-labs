...
logging.basicConfig(filename=str(LOG_DIR / 'plati_market_parser.log'), level=logging.INFO, 
...
def parse_product_page(url):
    try:
        ...
        if len(parts) < 3:
            logging.warning("Unexpected sell-count format on %s: %s", url, sales_node.text)
            return None
        sales = parts[1].strip()
...
def parse_seller_page(url):
    try:
        ...
        logging.error(f"Network error while parsing seller page {url}: {str(e)}")
        return None
